  ...
  return (
    <div>
      ...
      <div className="bg-card border border-border rounded-lg p-4 mb-6 flex gap-3 items-center">
        <input type="file" ref={fileRef} className="text-sm" onChange={upload} disabled={uploading} />
          ...
          {media.map((m) => (
            <div key={m.id} className="bg-card border border-border rounded-lg p-3">
              {m.mimeType?.startsWith('image/') ? (
                <img src={m.path} alt={m.filename} className="w-full h-32 object-cover rounded mb-2" />
              ) : (
              ...
              <div className="text-xs text-muted-foreground mt-0.5">{m.mimeType} · {formatBytes(m.sizeBytes)}</div>
              <div className="flex gap-2 mt-2">
                <button onClick={() => copyPath(m.path)} className="text-xs text-primary hover:underline">Copy path</button>
